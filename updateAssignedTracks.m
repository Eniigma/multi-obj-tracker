function [tracks] = updateAssignedTracks(assignments,tracks,timeWindowSize,centroids,bboxes,scores)
...
%               T = min(size(tracks(trackIdx).bboxes,1), 1);
%               w = mean([tracks(trackIdx).bboxes(end-T+1:end, 3); bbox(3)]);
%               h = mean([tracks(trackIdx).bboxes(end-T+1:end, 4); bbox(4)]); 
%             tracks(trackIdx).bboxes = [centroid - [w, h]/2, w, h];
            tracks(trackIdx).bboxes = bbox;
